...
def remove_cropped(volume):
    ...
    remove_labels = numpy.unique(numpy.concatenate((volume[:, :, :3], volume[:, :, -3:])))
...
if __name__ == '__main__':
    ...
    from scripts.post_processing.connected_component import remove_small
    ...
    for fold in range(1, config.num_folds + 1):
        ...
        for dataset, patients in valid_dataset_patients.items():
            for patient in track(patients, desc=f'Fold {fold} {dataset}'):
                volume_path = os.path.join('outputs', experiment_name, f'Fold_{fold}', dataset, patient, f'{Mode.WATERSHED}.npy')
                volume = numpy.load(volume_path)
                volume = remove_outlier(volume)
                volume = remove_cropped(volume)
                volume = remove_small(volume, voxel_threshold)
